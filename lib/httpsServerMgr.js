...
class httpsServerMgr {
  ...
  getSharedHttpsServer(hostname, proxyAuth) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self.httpsAsyncTask.addTask(taskName, prepareServer, (error, serverInfo) => {
        if (error) {
          reject(error);
        } else {
          resolve(serverInfo);
