...
const http = require('http'),
  ...
  getWsHandler = require('./handlers/wsHandler');
...
function getUserReqHandler(userRule, recorder) {
  ...
  return function (req, userRes) {
    ...
    */
    ...
    const fullUrl = (protocol === 'http' || (/^https:/).test(req.url)) ? req.url : (protocol + '://' + host + req.url);
...
function getConnectReqHandler(userRule, recorder, httpsServerMgr) {
  ...
  return function (req, cltSocket, head) {
    const proxyAuthHeader = req.headers['proxy-authorization'] || req.headers['Proxy-Authorization']
